...
const loadProductsByCategory = async (categoryId) => {
  try {
    ...
    const productList = raw
      .filter(pc => pc && typeof pc === "object" && pc.product && typeof pc.product === "object")
      .map(pc => {
        const p = pc.product;
        return {
          id: p.productId ?? p.id,            // tolerate either productId or id
          name: p.name,
          brand: p.brand,
          price: p.price,
          image: p.imageUrl ?? p.image,      // tolerate imageUrl or image
          stock: p.stock ?? 0
        };
      });
